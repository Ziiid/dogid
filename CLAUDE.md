...
Detta är "Dogsona" (tidigare arbetsnamn "Dog ID") — ett fristående syskonprojekt till [dogish](../dogish/), men **eget repo utan delad kod, databas eller backend**. Skälet: dogish-repot triggar automatisk deploy till Vercel/Railway vid push till main, och att blanda in ett obesläktat experiment där ökar bara risken att råka störa den pipelinen.
...
- **"Dog", inte "Pet".** Håller v1 smalt för att bevara den enkla kopplingen till hund-konceptet och undvika generiska fält som passar sämre för olika djurslag. Bredda till katt/andra djur senare bara om konceptet visar sig fungera.
...
- Möjlig PDF/kort-överlappning med dogish finns (dogish har en Premium-låst PDF-export med foderlogg/viktkurva/vaccinationsstatus), men den delen bygger på pågående loggad data som Dogsona aldrig kommer ha utan backend — bedömd risk för kannibalisering av Premium-uppgraderingar är låg.
...
- **Namn: "Dogsona"** (bytt från arbetsnamnet "Dog ID" 2026-07-21), persona-koppling som passar konceptet med flera valbara kort-"skepnader". Ingen "by X"-tagline i appen — headern visar bara logga + wordmark ("Dog" i grönt, "sona" i svart, samma tvåtonsmönster som dogish-wordmarken). App-ikon och splashscreen-logga är `dogsona.png` (repo-root).
- **Använd samma färgpalett som dogish** för visuell varumärkeskoppling. Dogish hex-koder (hårdkodade i `App.css`, inga CSS-variabler):
