...
## Fler sticker-omgångar, bakgrunds-toggle och kortstorlekar (2026-07-24)

**Fyra nya sticker-sheets importerade** (`hatts1.png`, `hatts2.png`, `emoji1.png`, `coffee.png`, `stickers3.png`) med samma connected-component-pipeline. Två nya tekniska varianter av samma grundproblem:
- `hatts1`/`hatts2`/`emoji1` hade **ingen äkta alfakanal alls** (RGB, schackmönster inbränt i pixlarna som ett rent visuellt konventionstecken för transparens) — löst med en syntetisk alfamask byggd på ljusstyrka+mättnad mot de två schackrutefärgerna.
- `stickers3` hade istället en **solid enfärgad bakgrund** (inte schackmönster) — enklare att maskera med ett rent färgavstånd från en referensfärg, och gav dessutom hela stickers i ett enda sammanhängande stycke (ingen kontur/fyllnad-fragmentering att slå ihop, till skillnad från schackmönster-bilderna).
- **Radgruppering på `round(y/100)` är opålitlig** när enskilda stickers har smala utskott (sugrör, horn, hatt-fjädrar) som sträcker sig långt utanför själva "kroppen" — bounding-boxens y-min hamnar då i fel rad-bucket och sorteringen blandas. Löst genom att hårdkoda rad-tillhörighet efter x-position istället för att lita på automatisk klustring när sådana fall upptäcks.
- 46 nya godkända stickers totalt (86 stickers totalt i appen nu) - se roadmap.md för hela listan per omgång. Kasserade pga stavfel/dubbletter: se respektive sessionslogg, samma mönster som tidigare (AI-genererad text i rastergrafik kan inte rättas i efterhand).

**Alla sex kvarvarande handritade SVG-stickers borttagna** (solglasögon, keps, sombrero, pratbubbla, hjärta, skrattande emoji) till förmån för riktiga bildstickers — `sombrero` och `heart`/`speechBubble` fick nya PNG-varianter på **samma sticker-id**, så gamla sparade placeringar bevarades. `pawPrints.png` visade sig dessutom vara ett människofotavtryck (inte en tass) och byttes ut i samma veva.

**Stickers har en vit "die-cut"-kant/skugga inbränd i varje bild (medveten ChatGPT-stil), stör mest på hattar.** Testade att erodera alfakanalen för att skala bort kanten (`ImageFilter.MinFilter`) — fungerade tekniskt men åt sig samtidigt in i själva konstverket där tunna mörka linjer låg nära kanten (t.ex. ett hack rakt genom polishattens skärm-skugglinje). Ingen global "trimma X pixlar"-lösning duger utan per-bild-justering. Beslut: låt befintliga stickers vara, be ChatGPT om rena bilder utan kant för framtida omgångar istället.

**Bakgrundsborttagning: från en engångs-toggle till en riktig fram-och-tillbaka-växel.** Första försöket (en kryssruta i formuläret som avgjorde borttagning *innan* fotot valdes) löste inte det faktiska behovet — användaren ville kunna ta bort/återställa bakgrunden upprepade gånger *efter* att fotot redan valts. Löst genom att spara både originalfotot och den bakgrundsborttagna versionen som separata filer (`dogid_photo_original.png`, `dogid_photo_nobg.png` i `dogStorage.js`), plus det "aktiva" fotot (`dogid_photo.png`, samma fil alla andra komponenter redan läser). En knapp i formuläret ("Ta bort bakgrund" / "Återställ bakgrund", text beror på aktuellt state) kopierar bara rätt cachad fil till den aktiva — kör Vision-ramverket bara första gången en riktning behövs, inte vid varje växling.

**Formulärets drag-i-cirkel-positionering var en kvarleva utan effekt — borttagen.** `dog.photoPositionX/Y` (drag för att positionera fotot i den runda förhandsvisningen) lästes aldrig av CardView eller något av korten — varje kort har sin egen oberoende `useDraggablePhoto`-transform som alltid börjar om från mitten. Kvarleva från den ursprungliga "ID-kort med runt foto"-idén före scope-skiftet 2026-07-22. Tog bort drag-interaktionen helt (pekar-handlers, `clamp`-hjälpfunktion, `photoDragHint`-strängen) och bytte samtidigt förhandsvisningen från cirkel+`object-fit: cover` till rundad fyrkant+`object-fit: contain`, så hela fotot syns okuggat istället för att osynligt beskäras mot ett läge man inte längre kan justera.

**Wanted fick sticker-stöd (fanns inte alls tidigare) och en mycket större fotoyta.** `.wanted-photo-frame` gick från 68%-bred kvadrat till 92% bredd med samma 4:5-proportion som Mugshot. Fotots drag-lyssnare flyttades samtidigt till ett eget `.wanted-photo-wrap`-syskon-element (samma fix som Studio-bugen 2026-07-23 — annars hade sticker-overlayen fått samma bubbling-krock). Ny lagring: `dog.wantedStickers`, `wanted: 'wantedStickers'` tillagt i `STICKER_FIELD_BY_TEMPLATE` (`CardView.jsx`).

**Lärdom: gissa inte CSS-dimensioner i huvudet, kopiera de faktiska värdena.** Två raka försök att få Studio-kortets totalhöjd att matcha Mugshot/Wanted via ett handräknat `aspect-ratio`-tal (först 2/3, sedan ett försök till) missade målet — `rem`-baserad text/padding skalar inte proportionellt med ett `aspect-ratio` som är bundet till kortets `vw`-baserade bredd, så matchningen glider beroende på skärmbredd. Löst genom att sluta gissa: en osynlig `.blank-footer` i Studio-kortet återanvänder **exakt samma padding/font-size/line-height-värden** som `.mugshot-placard`/`.mugshot-row` (kopierade rakt av, inte omräknade), vilket garanterar samma totalhöjd genom konstruktion istället för genom uträkning. Ett confused försök att lösa det genom att starta en headless webbläsare (`chromium-cli`) för att mäta pixlar avbröts på användarens begäran — hör inte hemma i det här arbetsflödet, gå tillbaka till CSS direkt.
